    ...
    void player_list::render_player_list()
    {
        if (ImGui::BeginTabItem("Player List"))
        {
            ...
            if (ImGui::BeginMenu(xorstr("Player Info")))
            {
                ...
                ImGui::Separator();
                ImGui::Text("Spectating : %s", isSpectating ? "True" : "False");
                ImGui::Separator();
            ...
            if (ImGui::CollapsingHeader("Other"))
            {
                ...
                if (ImGui::BeginMenu("Vehicle Spawner"))
                {
                    ...
                    ImGui::PushItemWidth(250);
                    ImGui::Combo("##ListVeh", &SelectedVehicle, var::VechicleList, IM_ARRAYSIZE(var::VechicleList)); // The second parameter is the label previewed before opening the combo.
                    ImGui::PopItemWidth();
                    if (ImGui::Button("Spawn Native"))
                    {
                        vehicle_helper::vehicle(var::VechicleList[SelectedVehicle], g_selected.ped);
                    }
                    ImGui::SameLine();
                ...
                if (ImGui::BeginMenu("Ped Spawner"))
                {
                    ...
                    ImGui::Checkbox("Invisible Ped", &IsVisible);
                    ImGui::SameLine();
                    ImGui::Checkbox("Aggresive Ped", &AggressivePed);
                    ImGui::Checkbox("Body Guard", &AsBodyGuard);
