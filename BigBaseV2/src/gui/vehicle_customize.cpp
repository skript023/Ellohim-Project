...
#include "vehicle_customize.h"

namespace big
{
    const char* lsc::get_mod_slot_name(int mod_slot, Vehicle vehicle)
    {
    ...
    const char* lsc::get_mod_name(int mod, int mod_slot, int mod_count, Vehicle vehicle)
    {
        if (mod_count == 0)
            return "";
        ...
        if (mod_slot == MOD_FRONTWHEEL || mod_slot == MOD_REARWHEEL)
        {
            ...
            if (mod >= mod_count / 2)
                ...
                return fmt::format("Chrome {}", HUD::_GET_LABEL_TEXT(VEHICLE::GET_MOD_TEXT_LABEL(vehicle, mod_slot, mod))).c_str();
            else
        ...
        {
        case MOD_ARMOR:
            return HUD::_GET_LABEL_TEXT(fmt::format("CMOD_ARM_{}", (mod + 1)).c_str());
        case MOD_BRAKES:
            return HUD::_GET_LABEL_TEXT(fmt::format("CMOD_BRA_{}", (mod + 1)).c_str());
        case MOD_ENGINE:
            if (mod == -1)
                return HUD::_GET_LABEL_TEXT("CMOD_ARM_0");
            return HUD::_GET_LABEL_TEXT(fmt::format("CMOD_ENG_{}", (mod + 1)).c_str());
        case MOD_SUSPENSION:
            return HUD::_GET_LABEL_TEXT(fmt::format("CMOD_SUS_{}", (mod + 1)).c_str());
        case MOD_TRANSMISSION:
            return HUD::_GET_LABEL_TEXT(fmt::format("CMOD_GBX_{}", (mod + 1)).c_str());
        }
        if (mod > -1)
        {
    ...
    const char * const xenonColourCaptions[] = {
    "Stock",
    "White",
    "Blue",
    "Electric Blue",
    ...
    void lsc::render_lsc_tab(const char* tab_name)
    {
        ...
        if (!slot_display_names.empty())
        {
            if (ImGui::BeginTabItem(tab_name))
            {
                ...
                if (ImGui::Combo("##XenonColour", &selected_xenon, xenonColourCaptions, IM_ARRAYSIZE(xenonColourCaptions)))
                {
                    g_fiber_pool->queue_job([]{
                        VEHICLE::_SET_VEHICLE_XENON_LIGHTS_COLOR(player_vehicle, selected_xenon-1);
                    });
                }
                    ...
                    case 6: //Interior Color
                    {
                        for (int i = 0; i < 74; i++)
                        {
                            if (ImGui::Selectable(classic_names[i], color_check == classic_ids[i]))
                            {
                                ...
                                g_fiber_pool->queue_job([] {
                                    VEHICLE::_SET_VEHICLE_INTERIOR_COLOR(player_vehicle, interior_color);
                                    });
                            }
                        }
                        break;
                    ...
                    case 7: //Dashboard Color
                    {
                        for (int i = 0; i < 74; i++)
                        {
                            if (ImGui::Selectable(classic_names[i], color_check == classic_ids[i]))
                            {
                                ...
                                g_fiber_pool->queue_job([] {
                                    VEHICLE::_SET_VEHICLE_DASHBOARD_COLOR(player_vehicle, dashboard_color);
                                    });
                            }
                        }
                        break;
