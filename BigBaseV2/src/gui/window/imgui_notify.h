...
#pragma warning(disable:4172 4715)
...
enum ImGuiToastType_
{
	...
	ImGuiToastType_Protection,
	ImGuiToastType_ReportBlock,
	ImGuiToastType_Ellohim
	...
	NOTIFY_INLINE auto get_default_title() -> char*
	{
		if (!strlen(this->title))
		{
			...
			case ImGuiToastType_Protection:
				return "Ellohim Protection";
			case ImGuiToastType_ReportBlock:
				return "Ellohim Block Report";
			case ImGuiToastType_Ellohim:
				return "Ellohim Mod Menu";
			}
		}
...
namespace ImGui
{
	...
	{
		const auto vp_size = *big::g_pointers->m_screen_resolution;
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			SetNextWindowPos(ImVec2((float)vp_size.x - NOTIFY_PADDING_X, (float)vp_size.y - NOTIFY_PADDING_Y - height), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);

			// Here we render the toast content
			{
				PushTextWrapPos((float)vp_size.x / 3.f); // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
	...
	NOTIFY_INLINE VOID MergeFont(void* font, int sizeof_font, float font_size, bool FontDataOwnedByAtlas = false)
	{
		ImFontConfig config;
